...
import {
  approveAwardComment,
  approveAwardComments,
  deleteAwardComment,
  deleteAwardComments,
  rejectAwardComment,
  rejectAwardComments,
  useAwardComments,
  type AwardCommentRecord,
} from '../model/awardCommentStore';
...
export function AwardCommentPanel({ awardId, commentsNeedAudit }: { awardId: number; commentsNeedAudit: boolean }) {
  const { message, modal } = App.useApp();
  const { promptReject, rejectReasonModal } = useRejectReasonPrompt();
  const data = useAwardComments(awardId);
  ...
  const selectedApprovableIds = filtered
    .filter((item) => selectedRowKeys.includes(item.id) && (item.status === '待审核' || item.status === '已驳回'))
    .map((item) => item.id);

  const deleteOne = (record: AwardCommentRecord) => {
    modal.confirm({
  ...
  const approveOne = (record: AwardCommentRecord) => {
    modal.confirm({
      ...
      footer: modalFooter,
      onOk: () => {
        if (approveAwardComment(record.id)) {
  ...
  const rejectOne = (record: AwardCommentRecord) => {
    promptReject({
      title: `确认驳回「${record.author}」的评论？`,
      description: '驳回后仅评论作者本人可见。',
      onConfirm: (reason) => {
        if (rejectAwardComment(record.id, reason)) {
  ...
  const statusColor: Record<AwardCommentRecord['status'], string> = {
    已通过: 'success',
    待审核: 'warning',
    已驳回: 'error',
  };

  const columns: TableColumnsType<AwardCommentRecord> = [
    ...
    },
    {
      ...
      dataIndex: 'status',
      width: 100,
      render: (value: AwardCommentRecord['status']) => <Tag color={statusColor[value]}>{value}</Tag>,
    },
    { title: '评论时间', dataIndex: 'createdAt', width: 180 },
    {
      ...
      render: (_, record) => (
        <Space>
          {commentsNeedAudit && (record.status === '待审核' || record.status === '已驳回') ? (
            <Button type="link" aria-label={`通过 ${record.author} 的评论`} onClick={() => approveOne(record)}>
              通过
            </Button>
          ) : null}
          {commentsNeedAudit && record.status === '待审核' ? (
      ...
      <Card>
        {selectedRowKeys.length ? (
          <Flex className="batch-toolbar" justify="space-between" align="center">
            ...
            <Space>
              {commentsNeedAudit && selectedApprovableIds.length ? (
                <Button
                  type="primary"
                  onClick={() =>
                    modal.confirm({
                      ...
                      footer: modalFooter,
                      onOk: () => {
                        const count = approveAwardComments(selectedApprovableIds);
              ...
              {commentsNeedAudit && selectedPendingIds.length ? (
                <Button
                  onClick={() =>
                    promptReject({
                      ...
                      description: '驳回后仅评论作者本人可见。',
                      onConfirm: (reason) => {
                        const count = rejectAwardComments(selectedPendingIds, reason);
                ...
                onClick={() =>
                  modal.confirm({
                    ...
                    onOk: () => {
                      ...
                      const removed = deleteAwardComments(ids);
