...
import { PcExamPrep } from './PcExamPrep';

describe('PC exam prep', () => {
  beforeEach(() => {
    __resetExamStoreForTests();
    __resetExamResultsForTests();
  });

  it('uses course PC detail layout with side CTA', () => {
    const html = renderToStaticMarkup(<PcExamPrep id={7} />);

    expect(html).toContain('class="c-pc-shell is-exam is-prep"');
    expect(html).toContain('<h1 class="c-pc-header-title">考试准备</h1>');
    expect(html).toContain('class="c-pc-detail"');
    expect(html).toContain('class="c-pc-side"');
    expect(html).toContain('class="c-detail-body c-article-body is-no-cover"');
    expect(html).toContain('class="c-detail-info-card"');
    expect(html).toContain('需求分析与PRD撰写能力考核');
    expect(html).toContain('总分：10分');
    expect(html).toContain('及格分：6分');
    expect(html).toContain('总题数：10题');
    expect(html).toContain('考试时长：100分钟');
  ...
  it('disables start before the exam start time', () => {
    ...
    expect(html).not.toContain('href="#/c/exam/7/take"');
    expect(html).not.toContain('>开始考试<');
  });

  it('is mounted from CEndApp exam prep page', () => {
    const html = renderToStaticMarkup(<CEndApp surface="pc" h5Page="exam-prep" examId={7} />);
    expect(html).toContain('class="c-pc-shell is-exam is-prep"');
