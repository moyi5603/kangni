...
import { canReviewActivity, type Activity } from '../model/activity';
import { reviewActivity } from '../model/activityStore';
...
export function ActivityReviewModal({
  ...
  const decide = async (pass: boolean) => {
    ...
    if (!activity || !canReviewActivity(activity)) {
      message.info('当前活动不可审核');
      return;
    }
    const result = reviewActivity(activity.id, pass ? '已通过' : '已驳回', comment, dayjs().format('YYYY-MM-DD HH:mm:ss'));
    if (!result) {
